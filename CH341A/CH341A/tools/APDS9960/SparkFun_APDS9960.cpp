...
void SleepWithMsgProcessing(unsigned int delay)
{
	Sleep(delay);
...
int SparkFun_APDS9960::readGesture()
{
    ...
    while(1) {
        ...
        SleepWithMsgProcessing(FIFO_PAUSE_TIME);
...
{
	if (ch341a.I2CWriteByte(APDS9960_I2C_ADDR, val))
		return false;
	return true;
}
...
{
	if (ch341a.I2CWriteCommandWriteByte(APDS9960_I2C_ADDR, reg, val))
		return false;
	return true;
}
...
bool SparkFun_APDS9960::wireWriteDataBlock(  uint8_t reg, 
                                        ...
                                        unsigned int len)
{
	if (ch341a.I2CWriteCommandWriteBytes(APDS9960_I2C_ADDR, reg, val, len))
		return false;
	return true;
}
...
{
	if (ch341a.I2CWriteCommandReadByte(APDS9960_I2C_ADDR, reg, val))
		return false;
    return true;
}
...
int SparkFun_APDS9960::wireReadDataBlock(   uint8_t reg,
                                        ...
                                        unsigned int len)
{
//	if (I2CWriteCommandReadBytes(uint8_t i2cAddr, uint8_t command, uint8_t *data, unsigned int count)
