...
public:
	...
	enum StatusCode {
		...
		STATUS_MF_TIMEOUT			= 4,	// Timeout in communication.
...
#ifdef __BORLANDC__
	void PICC_DumpToStr(Uid *uid, AnsiString &str);
	void PICC_DumpMifareClassicToStr(Uid *uid, byte piccType, MIFARE_Key *key, AnsiString &str);
	void PICC_DumpMifareClassicSectorToStr(Uid *uid, MIFARE_Key *key, byte sector, AnsiString &str);
	void PICC_DumpMifareUltralightToStr(AnsiString &str);
#else
